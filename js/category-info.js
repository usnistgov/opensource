app.controller('gitHubDataController', function($scope, $http, $window, Category) {
    var getCategoryInfo = $http.get('https://pages.nist.gov/opensource/category/category_info.json', {
        cache: true
    });

    var getReposTopics = $http.get('https://pages.nist.gov/opensource/explore/github-data/labRepos_Topics.json', {
        cache: true
    });

    var getReposInfo = $http.get('https://pages.nist.gov/opensource/explore/github-data/labReposInfo.json', {
        cache: true
    });

    var getReposLogos = $http.get('https://pages.nist.gov/opensource/assets/images/logos/repo_logos.json', {
        cache: true
    });
        ...
        getReposTopics.then(function(response) {
            ...
            getReposLogos.then(function(response) {
                ...
                $scope.repoHref = function(nametag) {
                    ...
                    $(location).attr('href', cpath('repo/#/' + nametag));
                };
